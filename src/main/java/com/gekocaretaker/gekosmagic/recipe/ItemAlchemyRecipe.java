...
public record ItemAlchemyRecipe(RegistryEntry<Item> from, RegistryEntry<Essence> ingredient,
                                RegistryEntry<Item> to) implements Recipe<AlchemyRecipeInput> {
    @Override
    public boolean matches(AlchemyRecipeInput input, World world) {
        return input.getItemStack().itemMatches(this.from) && input.getEssenceContainer().isOf(this.ingredient);
    }

    @Override
    public ItemStack craft(AlchemyRecipeInput input, RegistryWrapper.WrapperLookup registries) {
        return ElixirContentsComponent.createStack(this.to.value(), input.getContents(), input.getItemStack().getCount());
    }
