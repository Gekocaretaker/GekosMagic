...
public record ElixirRecipe(RegistryEntry<Elixir> from, RegistryEntry<Essence> ingredient,
                           RegistryEntry<Elixir> to) implements Recipe<AlchemyRecipeInput> {
    @Override
    ...
    public ItemStack craft(AlchemyRecipeInput input, RegistryWrapper.WrapperLookup registries) {
        return ElixirContentsComponent.createStack(input.getItemStack().getItem(), this.to, input.getItemStack().getCount());
    }
